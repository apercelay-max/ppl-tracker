    ...
    return (
          <div style={container}>
                <div style={scroll}>
                        ...
                        <div style={headerRow}>
                                  <button onClick={onBack} style={backBtn} aria-label="Retour">←</button>button>
                                  <div>
                                              <h1 style={title}>⚖️ Poids</h1>h1>
                                              <p style={{ color: 'var(--text-muted)', fontSize: 12, marginTop: 2 }}>Fonctionnalité en essai — dis-moi ce que t'en penses</p>p>
                                  </div>div>
                        </div>div>
                        ...
                        <div style={card}>
                                  ...
                                  <p style={fieldLabel}>MASSE GRAISSEUSE (facultatif)</p>p>
                                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                              <input
                                                              type="number"
                                                              inputMode="decimal"
                                  ...
                                  <p style={fieldLabel}>MASSE MUSCULAIRE (facultatif)</p>p>
                                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                              <input
                                                              type="number"
                                                              inputMode="decimal"
                                  ...
                                  <button onClick={handleAdd} style={addBtnFull}>Enregistrer</button>button>
                        
                          {last && (
                        <div style={{ marginTop: 12 }}>
                                      <p style={{ color: 'var(--text-dim)', fontSize: 11 }}>
                                                      Dernière pesée : <strong style={{ color: 'var(--text-secondary)' }}>{last.weightKg} kg</strong>strong> ({formatDate(last.date)})
                                      </p>p>
                          {(last.bodyFat || last.muscleMass) && (
                                          <p style={{ color: 'var(--text-dim)', fontSize: 11, marginTop: 4 }}>
                                            {last.bodyFat && (
                                                                <>Masse graisseuse : <strong style={{ color: 'var(--text-secondary)' }}>{formatMass(last.bodyFat.value, last.bodyFat.unit)}</strong>strong></>>
                                                              )}
                                            {last.bodyFat && last.muscleMass && '  ·  '}
                                            {last.muscleMass && (
                                                                <>Masse musculaire : <strong style={{ color: 'var(--text-secondary)' }}>{formatMass(last.muscleMass.value, last.muscleMass.unit)}</strong>strong></>>
                                                              )}
                                          </p>p>
                                      )}
                        </div>div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {bodyWeightHistory.map((entry) => (
                                      <div key={entry.id} style={row}>
                                                      <div style={{ flex: 1 }}>
                                                                        <p style={{ color: 'var(--text-secondary)', fontSize: 14, fontWeight: 700 }}>{entry.weightKg} kg</p>p>
                                                        {(entry.bodyFat || entry.muscleMass) && (
                                                            <p style={{ color: 'var(--text-dim)', fontSize: 11, marginTop: 2 }}>
                                                              ...
                                                              {entry.muscleMass && `Muscle ${formatMass(entry.muscleMass.value, entry.muscleMass.unit)}`}
                                                                        ...
                                                                        <p style={{ color: 'var(--text-dim)', fontSize: 11, marginTop: 2 }}>{formatDate(entry.date)}</p>p>
                                                      </div>div>
                                                      <button onClick={() => deleteBodyWeightEntry(entry.id)} style={deleteBtn}>✕</button>button>
                                      </div>div>
                                    ))}
                      </div>div>
                        )}
